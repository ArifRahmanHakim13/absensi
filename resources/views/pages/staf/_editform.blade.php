<div class="form-group row">
  <label for="name" class="col-sm-2 col-form-label required">Nama Staf</label>
  <div class="col-sm-10">
    <input type="text" name="name"
      value="{{ old('name', $staf->user->name) }}"
      class="form-control @error('name') is-invalid @enderror"
      id="name" placeholder="Ketik Nama Staf" required>
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <select name="jk" id="jk"
      class="form-control selectTwo @error('jk') is-invalid @enderror"
      data-width="100%" required>
      <option value="" disabled hidden>-- Pilih --</option>
      @foreach (['laki-laki','perempuan'] as $item)
        <option value="{{ $item }}"
          {{ old('jk', $staf->user->jk) == $item ? 'selected' : '' }}>
          {{ ucfirst($item) }}
        </option>
      @endforeach
    </select>
...
<!-- ✅ JABATAN SUDAH SESUAI ENUM DATABASE -->
<div class="form-group row">
  <label for="jabatan" class="col-sm-2 col-form-label required">Jabatan</label>
  <div class="col-sm-10">
    <select name="jabatan" id="jabatan"
      class="form-control selectTwo @error('jabatan') is-invalid @enderror"
      data-width="100%" required>
      <option value="" disabled hidden>-- Pilih --</option>
      @foreach ([
        'Dokter Umum',
        ...
        <option value="{{ $item }}"
          {{ old('jabatan', $staf->user->jabatan) == $item ? 'selected' : '' }}>
          {{ $item }}
        </option>
      @endforeach
    </select>
    @error('jabatan') <span class="invalid-feedback mt-1">{{ $message }}</span> @enderror
  </div>
</div>
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <input type="number" name="telepon"
      value="{{ old('telepon', $staf->user->telepon) }}"
      class="form-control @error('telepon') is-invalid @enderror"
      id="telepon" placeholder="Ketik Telepon/WA">
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <textarea name="alamat"
      ...
      id="alamat" placeholder="Ketik Alamat">{{ old('alamat', $staf->user->alamat) }}</textarea>
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <input type="text" name="username"
      value="{{ old('username', $staf->user->username) }}"
      class="form-control @error('username') is-invalid @enderror"
      id="username" placeholder="Ketik Username" required>
...
<div class="form-group row">
  ...
  <div class="col-sm-10 input-group">
    <input type="password" name="password"
      ...
      id="password" placeholder="Ketik Password Baru">
