...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <select name="periode" id="periode"
      ...
      @foreach (['1','2'] as $item)
        <option value="{{ $item }}"
          {{ old('periode', $tapel->periode) == $item ? 'selected' : '' }}>
          {{ $item == '1' ? 'Periode I' : 'Periode II' }}
        </option>
      @endforeach
    </select>
    @error('periode')
...
<input type="hidden"
       name="periode"
       value="{{ old('periode', $tapel->periode) }}">
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <input type="date"
           name="selesai"
           value="{{ old('selesai', $tapel->selesai) }}"
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <select name="is_aktif"
      ...
      @foreach (['1','0'] as $item)
        <option value="{{ $item }}"
          {{ old('is_aktif', $tapel->is_aktif) == $item ? 'selected' : '' }}>
          {{ $item == '1' ? 'AKTIF' : 'NON-AKTIF' }}
        </option>
      @endforeach
    </select>
    @error('is_aktif')
