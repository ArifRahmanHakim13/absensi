<div class="form-group row">
  ...
  <div class="col-sm-10">
    <input type="text" name="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Ketik Nama Kelas" required>
  ...
  <div class="col-sm-10">
    <select name="guru_id" id="guru_id" class="form-control selectTwo @error('guru_id') is-invalid @enderror" data-width="100%">
      ...
      @foreach ($guru as $item)
        <option value="{{ $item->id }}" {{ old('guru_id') == $item->id ? 'selected' : '' }}>{{ $item->user->name }}</option>
      @endforeach
    </select>
