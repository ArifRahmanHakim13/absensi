<div class="form-group row">
  ...
  <div class="col-sm-10">
    <input type="text" name="name" value="{{ old('name', $admin->user->name) }}" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Ketik Nama Admin" required>
  ...
  <div class="col-sm-10">
    <select name="jk" id="jk" class="form-control selectTwo @error('jk') is-invalid @enderror" data-width="100%" required>
      ...
      @foreach (['laki-laki','perempuan'] as $item)
        <option value="{{ $item }}" {{ old('jk', $admin->user->jk) == $item ? 'selected' : '' }}>{{ $item }}</option>
      @endforeach
    </select>
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <input type="number" name="telepon" value="{{ old('telepon', $admin->user->telepon) }}" class="form-control @error('telepon') is-invalid @enderror" id="telepon" placeholder="Ketik Telepon/WA">
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <textarea type="text" name="alamat" class="form-control @error('alamat') is-invalid @enderror" id="alamat" placeholder="Ketik Alamat" >{{ old('alamat', $admin->user->alamat) }}</textarea>
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <input type="text" name="username" value="{{ old('username', $admin->user->username) }}" class="form-control @error('username') is-invalid @enderror" id="username" placeholder="Ketik Username" required>
